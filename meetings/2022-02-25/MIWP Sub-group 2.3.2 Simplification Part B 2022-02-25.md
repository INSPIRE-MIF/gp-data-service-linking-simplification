...
**Agenda Item 2 - Discussion on issues**
   ...
   * Part B - Extended Capabilities elements remapping:
     * [Issue #39](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/39) (Conformity)
       ...
       * Both, @MarieLambois (FR) and @idevisser (NL) commented that this approach would be better, due to the fact that relying on the order of keywords is not safe when processing XML-kind files.
     ...
     * [Issue #40](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/40) (Temporal Reference)
       ...
       * @MarieLambois (FR) suggested to take any of the previous proposals as possible alternatives, either the one from @AntoRot (IT) or the one from @idevisser (NL).
       * @jescriu (JRC) mentioned that, if possible, it would be better to choose a solution which could support descentralized scenarios, where the service is managed by a different organization than the one creating or maintaining the dataset. He also mentioned that organisations need a common understanding on the meaning of the different possible types of date ('creation', 'publication' or 'revision'). All attendees agreed it would be useful to have recommendations in the TG MD.
       * @idevisser (NL) remarked that the objective of this execise was the simplification. She would promote a solution which avoids the need for updating the dataset metadata (by the dataset responsible party) after having updated the related service (by the service provider), while making metadata information trustable and correct. Often there are discrepancies between the information in the dataset (ISO) metadata and the one in the service Capabilities.
       * @MarieLambois (FR) finally suggested to take both of the previous proposals, first take the earliest publication date from the dataset metadata records, and additionally take - if available - the `UPDATESEQUENCE` revision date from the service Capabilities document (i.e. having one or two dates).
       * With the aim of simplifying, and keep only one date as mandatory in the metadata (as currently required in the TG MD), @AntoRot (IT) proposed to only consider the `UPDATESEQUENCE` from the service Capabilities document, but the issue is that the `UPDATESEQUENCE` seems not availbale in GeoServer.
       * Before resolving the (partial) disagreement on [Issue #40](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/40), all attendees agreed to first discuss [Issue #42](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/42), where the possible use of `UPDATESEQUENCE` was deeply analysed.
     ...
     * [Issue #42](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/42) (Metadata Date)
       ...
       * @heidivanparys (DK) will check the possibility to use an standardised date format according ISO 8601 for `UPDATESEQUENCE`.
     * [Issue #41](https://github.com/INSPIRE-MIF/gp-data-service-linking-simplification/issues/41) (Metadata Point of Contact)
